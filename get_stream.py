...
lock = threading.Lock()
music_choose_list = []
music_info_dict = {'hush':{'num':1, 'path':'music/hush.mp3', 'length':1}}
...
def download_music(word):
    ...
    try:
        ...
        f.close()
        print(f'下载{word}成功')
        music_info_dict[word]['path'] = f'music/{word}.mp3'
        music_info_dict[word]['length'] = 1
    except:
        print(f'下载{word}失败')
    sys.stdout.flush()
